...
# 或显式指定数据目录：
pwsh -File install/install.ps1 -DataDir "D:\DeepSeek-Harness\data"
```

**探测顺序**（未传 `-DataDir` 时）：环境变量 `DSH_HOME` → `D:\DeepSeek-Harness\data` → `%APPDATA%\dsh-desktop\data`。
...
Then **restart DSH** (fully quit from the tray). Detection order without `-DataDir`: `DSH_HOME` → `D:\DeepSeek-Harness\data` → `%APPDATA%\dsh-desktop\data`.
